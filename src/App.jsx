...
import { Home, Package, GitBranch, Shield, BarChart3, Menu, Bell, Settings, X } from 'lucide-react';
  ...
  return (
    <NotificationContext.Provider value={{ addNotification }}>
      ...
      <div className="fixed top-4 right-4 z-50 space-y-2 max-w-sm">
        {notifications.map(n => (
          <div key={n.id} className={`px-4 py-3 rounded-lg shadow-lg text-sm ${n.type === 'success' ? 'bg-green-500' : 'bg-red-500'} text-white animate-slide-in`}>
            {n.message}
          </div>
        ))}
      </div>
  ...
  return (
    <div className={`hidden lg:block ${isOpen ? 'w-64' : 'w-20'} bg-gray-900 text-white transition-all`}>
      <div className="p-4 border-b border-gray-800">
        {isOpen && <h1 className="text-xl font-bold text-blue-400">CoreIdentity</h1>}
        {!isOpen && <div className="text-xl font-bold text-blue-400">CI</div>}
      </div>
      <nav className="p-2 space-y-1">
        {nav.map(item => {
          const Icon = item.icon;
          const active = location.pathname === item.href;
          return (
            <Link key={item.name} to={item.href} className={`flex items-center px-3 py-2 rounded-lg ${active ? 'bg-blue-600' : 'hover:bg-gray-800'}`}>
              <Icon className="h-5 w-5" />
