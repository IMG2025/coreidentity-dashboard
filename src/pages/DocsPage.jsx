...
const ENDPOINTS = [
  {method:'POST', path:'https://portal.coreidentitygroup.com/api/agents/execute',          auth:true,  group:'Execution', desc:'Dispatch a governed agent execution through the Sentinel policy gate.',
  ...
  {method:'GET',  path:'https://portal.coreidentitygroup.com/api/agents/execute/executions',auth:true,  group:'Execution', desc:'Retrieve the governed execution audit log.',
  ...
  {method:'GET',  path:'https://portal.coreidentitygroup.com/api/agents/execute/stats',    auth:true,  group:'Execution', desc:'Aggregate governance stats across all clients.',
   response:'{"success":true,"totalExecutions":248,"avgGovernanceScore":94,"byClient":{"bank":{…},"health":{…}}}'},
  {method:'GET',  path:'https://portal.coreidentitygroup.com/api/telemetry/executions',    auth:true,  group:'Telemetry', desc:'Live telemetry execution feed from the in-memory ring buffer.',
  ...
  {method:'GET',  path:'https://portal.coreidentitygroup.com/api/telemetry/stats',         auth:true,  group:'Telemetry', desc:'Cross-client governance statistics and sync history.'},
  {method:'POST', path:'https://portal.coreidentitygroup.com/api/telemetry/ingest',        auth:true,  group:'Telemetry', desc:'Ingest telemetry from external sources or GCP services.',
  ...
  {method:'GET',  path:'https://portal.coreidentitygroup.com/api/live-data',               auth:true,  group:'Platform',  desc:'Aggregate live data from all 5 GCP services in a single request. Used by the Founders Dashboard.'},
  {method:'POST', path:'https://portal.coreidentitygroup.com/api/commercial/assess',       auth:false, group:'Commercial',desc:'Generate an instant governance assessment for a prospect.',
   body:'{"industry":"Healthcare","agentCount":18,"currentScore":58,"company":"Acme Hospital","email":"ciso@acme.com"}'},
  {method:'GET',  path:'https://portal.coreidentitygroup.com/api/commercial/pricing',      auth:false, group:'Commercial',desc:'Current pricing tiers and configuration.'},
  {method:'GET',  path:'https://portal.coreidentitygroup.com/api/commercial/report/:client',auth:true, group:'Commercial',desc:'Executive governance report for a client. :client = bank|health|retail|legal'},
];
...
export default function DocsPage() {
  ...
  useEffect(() => {
    fetch('https://portal.coreidentitygroup.com/api/commercial/docs')
      .then(r => r.json())
      .then(j => {
        if (j.endpoints) {
