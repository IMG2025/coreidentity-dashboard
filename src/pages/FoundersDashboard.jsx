...
function ConsolidatedView({viewMode}) {
  ...
  const trendData=buildTrend(CHC_REV,CHC_COST,CHC_TGT);
  const varData=buildVariance(CHC_REV,CHC_TGT);
  return (
    <div>
      ...
      {viewMode==='vs-targets' && (
        <>
          ...
          <div style={{marginTop:16, display:'flex', gap:12, flexWrap:'wrap'}}>
            {varData.slice(-3).map(d => (
              <div key={d.month} style={{background:C.surface, border:'1px solid '+C.border, borderRadius:6, padding:'10px 16px', minWidth:140}}>
                <div style={{color:C.slate, fontSize:11}}>{d.month}</div>
                <div style={{color:d.Variance>=0?C.green:C.red, fontFamily:'monospace', fontSize:14}}>{d.Variance>=0?'+':''}{fmtK(d.Variance)}</div>
                <div style={{color:C.slate, fontSize:11}}>{d.Variance>=0?'Beat':'Miss'} {Math.abs(Math.round(d.Variance/d.Target*100))}%</div>
              </div>
            ))}
          </div>
        </>
            ...
            <thead>
              <tr style={{borderBottom:'1px solid '+C.border}}>
                {['Entity','Type','Revenue','Costs','Net Profit','Margin','Agents'].map(h => (
                  <th key={h} style={{textAlign:'left', padding:'8px 12px', color:C.slate, fontSize:11, textTransform:'uppercase', letterSpacing:'0.08em'}}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
...
function CIAGView({viewMode}) {
  const curRet=last(CG_RET), curProj=last(CG_PROJ), curRev=last(CG_REV), curCost=last(CG_COST), curProf=curRev-curCost;
  ...
  return (
    <div>
      <div style={{display:'flex', gap:12, marginBottom:24, flexWrap:'wrap'}}>
        ...
        <MetricCard label="Retainer MRR"     value={fmtK(curRet)}     sub="4 active retainers"               accent={C.blue} />
        <MetricCard label="Project Revenue"  value={fmtK(curProj)}    sub="3 active projects"                accent={C.teal} />
        <MetricCard label="Net Profit"       value={fmtK(curProf)}    sub={Math.round(curProf/curRev*100)+'% margin'} accent={C.green} />
        <MetricCard label="Project Pipeline" value={fmtK(pipeline)}   sub="Total contracted value"           accent={C.gold} />
      </div>
      {viewMode==='trend' && (
        <>
  ...
  return (
    <div>
      <div style={{display:'flex', gap:12, marginBottom:24, flexWrap:'wrap'}}>
        ...
        <MetricCard label="Compliance Lift"   value="+29pts" sub="65% → 94% overall score"           accent={C.blue} trend={29} />
      </div>
      ...
      <div style={{background:C.surface, border:'1px solid '+C.border, borderRadius:8, padding:24, marginBottom:24}}>
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:24, marginBottom:24}}>
          ...
          <div>
            ...
            <div style={{color:C.white, fontSize:22, fontWeight:700, fontFamily:'monospace', marginBottom:4}}>
              23<span style={{color:C.slate, fontSize:13}}>/23 governed</span>
            </div>
      ...
      <div style={{borderBottom:'1px solid '+C.border, padding:'18px 32px', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
        ...
        <div style={{display:'flex', gap:24, alignItems:'center'}}>
          <div style={{textAlign:'right'}}>
            ...
            <div style={{color:C.gold, fontFamily:'monospace', fontSize:22, fontWeight:700}}>{fmtK(curRev)}</div>
          </div>
          <div style={{textAlign:'right'}}>
            <div style={{color:C.slate, fontSize:10, letterSpacing:'0.08em', textTransform:'uppercase'}}>Net Profit</div>
            <div style={{color:C.green, fontFamily:'monospace', fontSize:22, fontWeight:700}}>{fmtK(curProf)}</div>
          </div>
      ...
      <div style={{borderBottom:'1px solid '+C.border, padding:'0 32px', display:'flex'}}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            background:'none', border:'none', cursor:'pointer', padding:'14px 20px', fontSize:13, fontWeight:500,
            color:tab===t.id?C.gold:C.slate, borderBottom:tab===t.id?'2px solid '+C.gold:'2px solid transparent', transition:'color 0.15s'
          }}>{t.label}</button>
        ))}
      </div>
        ...
        <div style={{display:'flex', background:C.surface, border:'1px solid '+C.border, borderRadius:6, overflow:'hidden'}}>
          {VIEWS.map(v => (
            <button key={v.id} onClick={() => setViewMode(v.id)} style={{
              background:viewMode===v.id?C.border:'none', border:'none', cursor:'pointer', padding:'6px 16px',
              fontSize:12, color:viewMode===v.id?C.white:C.slate, transition:'all 0.15s'
            }}>{v.label}</button>
          ))}
        </div>
      </div>
