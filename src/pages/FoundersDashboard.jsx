// FoundersDashboard.jsx — LIVE DATA · Audit Trail
// Auto-generated by Script 24 — DO NOT hand edit
...
// ── Agents available for demo dispatch ───────────────────────
const DEMO_AGENTS = {
  bank:   [
...
function CoreIdentityView({vm, d}) {
  ...
  return (
    <div>
      ...
      {vm==='trend'      && (<><SecHdr title="CoreIdentity — 12-Month Trend" live={!!ci}/><ResponsiveContainer width="100%" height={240}><AreaChart data={trendData(ciRev,ciCost,ciTgt)}><defs><linearGradient id="ciG" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={C.blue} stopOpacity={0.3}/><stop offset="95%" stopColor={C.blue} stopOpacity={0}/></linearGradient></defs><CartesianGrid stroke={C.border} strokeDasharray="3 3"/><XAxis dataKey="month" tick={{fill:C.slate,fontSize:11}}/><YAxis tickFormatter={v=>'$'+(v/1000)+'K'} tick={{fill:C.slate,fontSize:11}}/><Tooltip content={<ChartTip/>}/><Legend wrapperStyle={{color:C.slate,fontSize:12}}/><Area type="monotone" dataKey="Revenue" stroke={C.blue} fill="url(#ciG)" strokeWidth={2}/><Line type="monotone" dataKey="Target" stroke={C.slate} strokeWidth={1} strokeDasharray="4 4" dot={false}/><Line type="monotone" dataKey="Costs" stroke={C.red} strokeWidth={1.5} dot={false}/></AreaChart></ResponsiveContainer></>)}
      {vm==='vs-targets' && (<><SecHdr title="CoreIdentity — vs Targets" live={!!ci}/><ResponsiveContainer width="100%" height={240}><BarChart data={varianceData(ciRev,ciTgt)}><CartesianGrid stroke={C.border} strokeDasharray="3 3"/><XAxis dataKey="month" tick={{fill:C.slate,fontSize:11}}/><YAxis tickFormatter={v=>'$'+(v/1000)+'K'} tick={{fill:C.slate,fontSize:11}}/><Tooltip content={<ChartTip/>}/><Legend wrapperStyle={{color:C.slate,fontSize:12}}/><Bar dataKey="Target" fill={C.border} radius={[2,2,0,0]}/><Bar dataKey="Actual" fill={C.blue} radius={[2,2,0,0]}/></BarChart></ResponsiveContainer></>)}
...
function CIAGView({vm, d}) {
  ...
  return (
    <div>
      ...
      {vm==='trend'      && (<><SecHdr title="CIAG — 12-Month Trend" live={!!cg}/><ResponsiveContainer width="100%" height={240}><AreaChart data={trendData(cgRev,cgCost,cgTgt)}><defs><linearGradient id="cgG" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={C.teal} stopOpacity={0.3}/><stop offset="95%" stopColor={C.teal} stopOpacity={0}/></linearGradient></defs><CartesianGrid stroke={C.border} strokeDasharray="3 3"/><XAxis dataKey="month" tick={{fill:C.slate,fontSize:11}}/><YAxis tickFormatter={v=>'$'+(v/1000)+'K'} tick={{fill:C.slate,fontSize:11}}/><Tooltip content={<ChartTip/>}/><Legend wrapperStyle={{color:C.slate,fontSize:12}}/><Area type="monotone" dataKey="Revenue" stroke={C.teal} fill="url(#cgG)" strokeWidth={2}/><Line type="monotone" dataKey="Target" stroke={C.slate} strokeWidth={1} strokeDasharray="4 4" dot={false}/><Line type="monotone" dataKey="Costs" stroke={C.red} strokeWidth={1.5} dot={false}/></AreaChart></ResponsiveContainer></>)}
      {vm==='vs-targets' && (<><SecHdr title="CIAG — vs Targets" live={!!cg}/><ResponsiveContainer width="100%" height={240}><BarChart data={varianceData(cgRev,cgTgt)}><CartesianGrid stroke={C.border} strokeDasharray="3 3"/><XAxis dataKey="month" tick={{fill:C.slate,fontSize:11}}/><YAxis tickFormatter={v=>'$'+(v/1000)+'K'} tick={{fill:C.slate,fontSize:11}}/><Tooltip content={<ChartTip/>}/><Legend wrapperStyle={{color:C.slate,fontSize:12}}/><Bar dataKey="Target" fill={C.border} radius={[2,2,0,0]}/><Bar dataKey="Actual" fill={C.teal} radius={[2,2,0,0]}/></BarChart></ResponsiveContainer></>)}
          ...
          return (
            <div key={i} onClick={() => setSelected(c)} style={{background:C.surface, border:'1px solid '+(selected?.id===c.id?cc:C.border), borderRadius:8, padding:20, cursor:'pointer', borderLeft:'3px solid '+cc}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:12}}>
                <div><div style={{fontSize:16,marginBottom:2}}>{CLIENT_ICONS[c.id]||'🏢'}</div><div style={{color:C.white,fontSize:13,fontWeight:600,marginBottom:2}}>{c.name}</div><div style={{color:C.slate,fontSize:11}}>{c.vertical}</div></div>
...
function AuditTrailView() {
  ...
  const fetchAudit = useCallback(async () => {
    try {
      const [execRes, statsRes] = await Promise.all([
        fetch(API_URL + 'https://portal.coreholdingcorp.com/api/telemetry/executions?limit=100', { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } }),
        fetch(API_URL + 'https://portal.coreholdingcorp.com/api/telemetry/stats', { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } })
      ]);
  ...
  useEffect(() => {
    fetch(API_URL + 'https://portal.coreholdingcorp.com/api/telemetry/seed', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}'  , 'Authorization': `Bearer ${localStorage.getItem('token')}` })
      .then(() => fetchAudit())
      .catch(() => fetchAudit());
  ...
  const fireAgent = async () => {
    ...
    try {
      ...
      const res    = await fetch(API_URL + 'https://portal.coreholdingcorp.com/api/agents/execute', {
        method: 'POST',
  ...
  return (
    <div>
      ...
      <div style={{display:'flex', gap:12, marginBottom:24, flexWrap:'wrap'}}>
        ...
        <Card label="Total Syncs"       value={(stats?.totalSyncs||0).toLocaleString()} sub="AWS↔GCP" accent={C.blue} />
        ...
        <div style={{display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:24}}>
          {Object.entries(stats.byClient).map(([cid, cs]) => (
            <div key={cid} style={{background:C.surface, border:'1px solid '+(CLIENT_COLORS[cid]||C.border)+'44', borderRadius:8, padding:'14px 16px', borderLeft:'3px solid '+(CLIENT_COLORS[cid]||C.gold)}}>
              <div style={{fontSize:16, marginBottom:4}}>{CLIENT_ICONS[cid]||'🏢'}</div>
      ...
      {loading && (
        <div style={{textAlign:'center', padding:40, color:C.slate}}>⏳ Loading execution log…</div>
      )}
                  ...
                  <tr key={e.executionId||i} style={{
                    ...
                    <td style={{padding:'10px 14px'}}>
                      <div style={{display:'flex', alignItems:'center', gap:6}}>
                        <span style={{fontSize:14}}>{CLIENT_ICONS[cid]||'🏢'}</span>
...
export default function FoundersDashboard() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res  = await fetch(API_URL + 'https://portal.coreholdingcorp.com/api/live-data', { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } });
      const json = await res.json();
      if (json.data) {
