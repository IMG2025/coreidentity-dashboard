...
function AuditTrailView() {
  ...
  const fetchAudit = useCallback(async () => {
    try {
      const [execRes, statsRes] = await Promise.all([
        fetch(API_URL + 'https://api.coreidentity.coreholdingcorp.com/api/telemetry/executions?limit=100', { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } }),
        fetch(API_URL + 'https://api.coreidentity.coreholdingcorp.com/api/telemetry/stats', { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } })
      ]);
  ...
  useEffect(() => {
    fetch(API_URL + 'https://api.coreidentity.coreholdingcorp.com/api/telemetry/seed', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}'  , 'Authorization': `Bearer ${localStorage.getItem('token')}` })
      .then(() => fetchAudit())
      .catch(() => fetchAudit());
...
export default function FoundersDashboard() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res  = await fetch(API_URL + 'https://api.coreidentity.coreholdingcorp.com/api/live-data', { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } });
      const json = await res.json();
      if (json.data) {
